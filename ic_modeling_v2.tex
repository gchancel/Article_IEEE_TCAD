...
%		The resistors values depends on the considered technology, as well as the capacitors values, and can be adjusted and calculated according to one needs.
	...
	\subsubsection{The resulting model}
		...
		\end{itemize}
		In addition to this, the desired substrate thickness can be achieved by instanciating as much as needed the elementary substrate layer inside the model itself, its thickness being eT \textmu m.
...
%	\subsubsection{Negative dual-well simulation results}
	\subsection{Simulation results: dual-well negative pulse}
		Fig. \ref{sim_res_dw_neg} shows the simulation results for a negative pulse aplpied to a dual-well IC
...
%	\subsubsection{Differences between dual-well and triple-well (negative and positive pulses)}
	\subsection{Simulations results: summary and conclusions}
		...
		In that case, what we mean by dangerousness is how fast we have observed a complete circuit destruction according to the voltage pulse polarity and the substrate type.
...
%	\textcolor{orange}{The memories, however, are made of a mix of both.}
	...
	\end{itemize}
	The voltage pulse used is of negative polarity as we have observed a very fast degradation of IC exposed to positive voltage pulses, therefore we decided to avoid them at all costs.
	...
	To circumvent this limitation, we decided to develop an addition to the initial simulation flow, hence the name "hybrid simulation flow".